...
while cv2.waitKey(delay) !=ord('q'):
    try:
        ...
        if not ret_val:
            ...
            img = cv2.flip(img, 1)
            frame = img
            img = imresize(img,[img_height, img_width],preserve_range=True)
            img = np.array([img])
            ...
            for i in y:
                ratio_width=img_width/original_width
                ratio_height=img_height/original_height
                x1=int(i[2]/ratio_width)
                y1=int(i[3]/ratio_height)
                x2=int(i[4]/ratio_width)
                y2=int(i[5]/ratio_height)
                frame=cv2.rectangle(frame, (x1,y1),(x2,y2), (0,255,0),1)
                
                frame = cv2.putText(frame, classes[int(i[0])], (100, 200), cv2.FONT_HERSHEY_SIMPLEX,
        ...
        # Insert FPS/quit text and show image
            ...
            frame = cv2.putText(frame, fps, (0, 20), cv2.FONT_HERSHEY_SIMPLEX,
                              fontScale=0.5, color=(0, 255, 255))
            frame = cv2.putText(frame, 'Press q to quit', (200, 20), cv2.FONT_HERSHEY_SIMPLEX,
